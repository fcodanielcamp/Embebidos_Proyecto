...
**Arquitectura de 3 Capas basada en Ganadería de Precisión (PLF)**[cite: 1]
...
La estructura se organiza por capas para garantizar la autonomía del desarrollo y la resiliencia operativa del sistema[cite: 1]:

| Carpeta / Ruta | Capa | Responsable | Tecnología Core |
| :--- | :--- | :--- | :--- |
| `/firmware` | **1. Campo (ESP32)** | Ing. de Firmware | C++, MQTT, RFID[cite: 1] |
| `/capas/borde_rpi` | **2. Borde (Rpi Zero 2W)** | Ing. de Redes/Software | Docker, Mosquitto, Node-RED[cite: 1] |
| `/capas/datos_pc` | **3. Datos (PC Central)** | Ing. de Datos | MariaDB, SQL, Grafana[cite: 1] |
...
3. **Configurar Credenciales**: 
   ...
   * **IMPORTANTE (Windows Fix)**: Crear un archivo vacío en `capas/borde_rpi/mosquitto/config/mosquitto.passwd`. Esto es mandatorio para que el contenedor de Mosquitto inicie correctamente en Windows debido a políticas de seguridad de volúmenes[cite: 1].

### 2. Despliegue de Servicios por Hardware
El despliegue se realiza de forma independiente según el nodo físico[cite: 1]:
...
*   **Misión**: Ruteo local de datos, gestión de HMI y ruteo de señales UDP[cite: 1].
*   **Configuración Obligatoria**: Configurar un **Archivo Swap de 2GB** en la Raspberry Pi Zero 2W para evitar el colapso del stack por falta de RAM (512MB)[cite: 1].
...
*   **Misión**: Desarrollo de interfaz táctil para reportes de salud y comandos de flujo inverso[cite: 1].
*   **Precisión Táctil**: Implementar una **Matriz de Homografía** para mapear coordenadas resistivas (0-4095) a la resolución de 240x320 píxeles[cite: 1].
...
*   **Misión**: Persistencia masiva en PC Central para proteger la integridad de la tarjeta SD de la Raspberry Pi[cite: 1].
*   **Esquema**: Tabla `registros_gallinas` diseñada para soportar timestamps de alta precisión (13 dígitos/epoch ms)[cite: 1].
*   **Acceso**: Configurar el firewall para permitir tráfico entrante en el puerto `3306` (MariaDB)[cite: 1].
...
*   **Broker**: `mosquitto_broker` (Puerto 1883)[cite: 1].
*   **Tópicos Críticos**:
...
*   **Puerto**: `3306` (Apuntando a la IP estática de la PC Central)[cite: 1].
*   **Reportes de Salud**: Los estados permitidos son `Saludable`, `Letárgica`, `Herida` y `Problema con alimento`[cite: 1].

---

## 🛠️ Reglas de Colaboración en Git
1. **Despliegue por Capas**: No mezclar servicios de la PC (MariaDB) en el despliegue de la Raspberry Pi para evitar saturación de RAM[cite: 1].
